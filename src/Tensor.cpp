...
Tensor Tensor::operator+(const Tensor& other) const {
    ...
    if (req_grad) {
        ...
        out_node->m_gradfn =
            [this_node, other_node, out_node, this_scalar,
    ...
    if (req_grad) {
        ...
        out_node->m_gradfn = [this_node, other_node, out_node, this_scalar,
            ...
            for (std::size_t i = 0; i < out_node->m_data.size(); ++i) {
                ...
                const float other_value = other_scalar ? other_node->m_data[0]
                                                       : other_node->m_data[i];
...
Tensor Tensor::matmul(const Tensor& other) const {
    ...
    if (this_dims == 2 && other_dims == 2) {
        ...
        throw std::invalid_argument(
            "matmul only supported for 1D or 2D tensors");
    }
             ...
             other_dims](const std::vector<float>& grad_output) {
                ...
                if (this_dims == 1 && other_dims == 2) {
                    ...
                    for (auto j = 0; j < k; ++j) {
                        ...
                        for (auto i = 0; i < m; ++i) {
                            if (this_node->m_requires_grad) {
                                this_node->m_grad[i] +=
                                    g * other_node->m_data[i * k + j];
                            }

                            if (other_node->m_requires_grad) {
                                other_node->m_grad[i * k + j] +=
                                    g * this_node->m_data[i];
                            }
                        }
                    }
                }
                ...
                if (this_dims == 2 && other_dims == 1) {
                    ...
                    for (auto j = 0; j < k; ++j) {
                        ...
                        for (auto i = 0; i < m; ++i) {
                            if (this_node->m_requires_grad) {
                                this_node->m_grad[j * m + i] +=
                                    g * other_node->m_data[i];
                            }

                            if (other_node->m_requires_grad) {
                                other_node->m_grad[i] +=
                                    g * this_node->m_data[j * m + i];
                            }
                        }
                    }
                }
