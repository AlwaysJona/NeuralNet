...
std::unordered_map<std::string, std::shared_ptr<Node>> load(
    ...
    while (file.peek() != EOF) {
        ...
        file.read(reinterpret_cast<char *>(shape.data()),
                  shape_length * sizeof(std::size_t));
